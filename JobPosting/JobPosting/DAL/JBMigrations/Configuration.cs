    ...
    internal sealed class Configuration : DbMigrationsConfiguration<JobPosting.DAL.JBEntities>
    {
        ...
        protected override void Seed(JobPosting.DAL.JBEntities context)
        {
            ...
            var postings = new List<Posting>
                        {
                           ...
                           , pstJobDescription = "this job will take all the skills of teaching and more as the Vice Principle you are required to" +
                            "look into many different fields of the school...", PositionID=(context.Positions.Where(p=>p.PositionCode == "10330").SingleOrDefault().ID)}
                        };
            ...
            var applications = new List<Application>
                                    {
                                       new Application { PostingID = 1,  ApplicantID=(context.Applicants.Where(p=>p.apEMail == "testuser@hotmail.com").SingleOrDefault().ID), Priority = 2},
                                       new Application {  PostingID = 1,  ApplicantID=(context.Applicants.Where(p=>p.apEMail == "testuser1@hotmail.com").SingleOrDefault().ID), Priority = 1},
                                       new Application {  PostingID = 1,  ApplicantID=(context.Applicants.Where(p=>p.apEMail == "testuser2@hotmail.com").SingleOrDefault().ID), Priority = 3}
            ...
            jobRequirements.ForEach(a => context.JobRequirements.AddOrUpdate(n => n.ID, a));
            SaveChanges(context);
            ...
            var applicationsCart = new List<ApplicationCart>
                        {
                            new ApplicationCart {  PostingID=1,  ApplicantID=(context.Applicants.Where(p=>p.apFirstName == "kevin").SingleOrDefault().ID), Priority = 1},
